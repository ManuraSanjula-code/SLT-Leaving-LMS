...
@RestController
@RequestMapping("/users/api/upload")
public class FileUploadController {
    ...
    @PostMapping("/csv")
    public ResponseEntity<LMSResponse> uploadCsv(@RequestParam("file") MultipartFile file) {
        try {
            List<UserEntity> users = parseCsv(file);
            return ResponseEntity.ok(processUsers(users));
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.badRequest().body(new LMSResponse("Error processing CSV file: " + e.getMessage()));
        }
    }
