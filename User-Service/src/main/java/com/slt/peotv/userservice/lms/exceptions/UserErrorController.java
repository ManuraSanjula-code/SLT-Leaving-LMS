...
public class UserErrorController extends AbstractErrorController {
    ...
    public ResponseEntity errorMapping(HttpServletRequest request){
        ...
        if((body.get("error").equals("Forbidden") && status.value() == Integer.valueOf((Integer) body.get("status"))) || (body.get("error").equals("Unauthorized") && status.value() == Integer.valueOf((Integer) body.get("status")))){//Not Found
