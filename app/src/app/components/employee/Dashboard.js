...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/lms/dashboard/${userId}/${userId}`, {
        method: 'GET',
        credentials: 'include', // This sends all cookies
        headers: {
  ...
  // Transform leave distribution data for pie chart
  const getLeaveDistributionData = () => {
    ...
    return Object.entries(dashboardData.remainLeaveDistribution)
        .filter(([_, value]) => value > 0) // Only show positive values
        .map(([name, value]) => ({ name, value }));
  };

  // Transform attendance data for bar chart
