...
public class LMS_Service_impl implements LMS_Service {
	...
	private AbsenteeDto convertToDto(AbsenteeEntity entity) {
		...
		dto.setIsFullDay(entity.getIsFullDay());
		dto.setIsSupervisedApproved(entity.getIsSupervisedApproved());
		dto.setIsHODApproved(entity.getIsHODApproved());
		dto.setAudited(entity.getAudited());
		dto.setIsNoPay(entity.getIsNoPay());
		dto.setIsPending(entity.getIsPending());
		dto.setIsAccepted(entity.getIsAccepted());
		dto.setIsLate(entity.getIsLate());
		dto.setIsAbsent(entity.getIsAbsent());
		dto.setIsUnSuccessfulAttdate(entity.getIsUnSuccessfulAttdate());
		dto.setIsLateCover(entity.getIsLateCover());
	...
	public Page<MovementDTO> getAllMovementByUser(String employeeId, int page, int size) {
		...
		return allByUser.map(movementEntity -> {
			...
			List<MovementAdminsDto> movementAdminsDtoList = new ArrayList<>();
			movementEntity.getAdmins().forEach(movementAdminsEntity -> {
				MovementAdminsDto movementAdminsDto = new MovementAdminsDto();
				movementAdminsDto.setId(movementAdminsEntity.getId());
				movementAdminsDto.setMovementId(movementAdminsEntity.getMovementId());
				movementAdminsDto.setUserId(movementAdminsEntity.getUserId());
				movementAdminsDto.setSltId(movementAdminsEntity.getSltId());
				movementAdminsDto.setEmployeeId(movementAdminsEntity.getEmployeeId());
				movementAdminsDto.setApprovedDate(movementAdminsEntity.getApprovedDate());
				movementAdminsDto.setHighestRolePriority(movementAdminsEntity.getHighestRolePriority());
				movementAdminsDto.setAccepted(movementAdminsEntity.getIsAccepted());
				movementAdminsDtoList.add(movementAdminsDto);
			});
			movementDTO.setMovementAdmins(movementAdminsDtoList);
